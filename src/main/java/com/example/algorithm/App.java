...
/**
 * 編譯：javac -encoding UTF-8 -d out App.java
 * 執行：java -cp out com.example.algorithm.App
 *
 * 不需要任何第三方套件。
 *
 * 分配規則：
 * 1. 每個模組恰好有 1 位主責，以及 ModuleRule 所設定的 1～2 位備援。
 * 2. 同一個模組的主責與備援一定是不同的人。
 * 3. 每人全部角色（主責＋備援）的總件數最大與最小最多相差 1。
 * 4. 每人的主責數最大與最小最多相差 1；全部備援數也最多相差 1。
 * 5. 高權重／高倍率／高責任比例的工作，會優先分給認為它較簡單的人。
 * 6. 最後以不破壞以上限制的交換，降低加權工作量的不平均程度。
 * 7. 獎懲權重 = 基礎權重 × 加級倍率 × 全員平均難度名次；
 *    再依主責／備援責任比例分攤。
 */
public class App {
